...
# Para la primera parte de la modulación 16-QAM lo que se hace es modificar las funciones modulador y demodulador para adaptarlas a la modulación requerida.

# In[9]:


'''Se modificarán las funciones modulador y demodulador para adaptarlas a la modulación 16-QAM.
El procedimiento es el mismo de la modulación BPSK, pero aplicado a dos portadoras de cuatro bits.'''

def modulador(bits, fc, mpp): 
    
    i = 0 # Se inicializa i en cero. Se usará como iterador más adelante
    
    # Construyendo un periodo de la señal portadora
    Tc = 1 / fc  # periodo
    t_periodo = np.linspace(0, Tc, mpp) 
    
    # Señales portadoras
    port1 = np.cos(2*np.pi*fc*t_periodo)
    port2 = np.sin(2*np.pi*fc*t_periodo)
    
    # Tiempo total de la simulacion
    t_simulacion = np.linspace(0, (len(bits))*Tc, (len(bits))*mpp)
    
    # Inicialización de la señal modulada
    senal_Tx = np.zeros(t_simulacion.shape)
    
    # Se hace un for para comparar paquetes de 2 bits
    for j in range(0, len(bits), 4):
        senal_Tx[i*mpp : (i+1)*mpp] =             (-1)**(1+bits[j]) * 3**(1-bits[j+1]) * port1             + (-1)**(bits[j+2]) * 3**(1-bits[j+3]) * port2
        i = i+1
    
    # Calcular la potencia promedio de la señal modulada
    P_senal_Tx = 1 / ((len(bits))*Tc) * np.trapz(pow(senal_Tx, 2), t_simulacion)

    return senal_Tx, P_senal_Tx, port1, port2

def demodulador(senal_Rx, port1, port2, mpp):
    # Cantidad de muestras en senal_Rx
    M = len(senal_Rx)
    ...
    # Vector para bits obtenidos por la demodulación
    bits_Rx = np.zeros(N)
    demodulada = np.zeros(senal_Rx.shape)
    j = 0
    ...
    for i in range(N):
        if j+4 > N:  
            ...
            prod1 = senal_Rx[i*mpp: (i+1)*mpp] * port1
            prod2 = senal_Rx[i*mpp: (i+1)*mpp] * port2
            demodulada[i*mpp: (i+1)*mpp] = prod1 + prod2
            
            # Criterio de decisión
            if np.sum(prod1) >= 0: 
                bits_Rx[j] = 1  # bit 1 
                
            if np.max(np.abs(prod1)) < 2.5: 
                bits_Rx[j+1] = 1  # bit 2
                
            if np.sum(prod2) < 0: 
                bits_Rx[j+2] = 1  # bit 3
                
            if np.max(np.abs(prod2)) < 2.5: 
                bits_Rx[j+3] = 1  # bit 4

            j += 4
    return bits_Rx.astype(int), demodulada

def print_img(img):
    fc = 5000   # frecuencia de la portadora
    mpp = 10    # muestras por periodo de la portadora
    SNR = 10    # relación señal-a-ruido del canal   

    # Iniciar medición del tiempo de simulación
    inicio = time.time()

    # Importar y convertir la imagen a transmitir
    imagen_Tx = fuente_info(img)
    dimensiones = imagen_Tx.shape
    
    # Codificar los pixeles de la imagen
    bits_Tx = rgb_a_bit(imagen_Tx)

    # Modular la cadena de bits usando el esquema 16-QAM
    senal_Tx, Pm, port1, port2 = modulador(bits_Tx, fc, mpp)
    string_Pm = "{:.4f}".format(Pm)
    print("Potencia de emisión:", string_Pm)
    
    # Se transmite la señal modulada, por un canal ruidoso
    senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)
    
    # Se desmodula la señal recibida del canal
    bits_Rx, demodulada = demodulador(senal_Rx, port1, port2, mpp)
    
    # Se visualiza la imagen recibida
    imagen_Rx = bits_a_rgb(bits_Rx, dimensiones)
    Fig = plt.figure(figsize=(10, 6))

    # Cálculo del tiempo de simulación
    sim_time = time.time()-inicio
    string_time = "{:.4f}".format(sim_time) + "s"
    print('Duración de la simulación: ', string_time)
    ...
    # La señal demodulada
    ax4.plot(demodulada[0:600], color='m', lw=2)
    ax4.set_ylabel('$b^{\'}(t)$')
    ax4.set_xlabel('$t$ / milisegundos')
    fig.tight_layout()
...
# 4 posibles valores para la amplitud
A = [-3, -1, 1, 3]                     
N =  16                               
Cmm = np.empty((N, len(t)))    # N funciones del tiempo x(t) con T puntos

# Para la señal se aplica cada valor de amplitud
for i in A:
...
P = [np.mean(Cmm[:,i]) for i in range(len(t))]
Rea, = plt.plot(t, P, lw=6, color = 'b', label='Promedio realizaciones')
...
for n in range(N):
    for i, tau in enumerate(desplazamiento):
        corr[n, i] = np.correlate(Cmm[n,:], np.roll(Cmm[n,:], tau))/T
    plt.plot(taus, corr[n,:])

# Valor teórico de correlación 
# La magnitud es (varianza - media^2) con p = 0.5 ya que es una variable de Bernoulli
Rxx = (0.5 - np.power(0.25,2)) * np.cos(np.pi*taus)

# Gráficas de correlación para cada realización
plt.plot(taus, Rxx, '-.', lw = 4, color = 'g', label = 'Correlación teórica')
